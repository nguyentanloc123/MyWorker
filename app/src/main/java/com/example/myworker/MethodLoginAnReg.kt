...
// Initialize Firebase Auth
    ...
    fun goMain(uses: String?, sdtt: String?,email: String?, password: String?)
    {
        ...
        {
            putString(getString(com.example.myworker.R.string.usernameEmail), email)
            commit()
        }
        with (sharedPref2.edit())
        {
            putString(getString(com.example.myworker.R.string.usernamePass), password)
            commit()
        }
        ...
        {
            putString(getString(com.example.myworker.R.string.usernameUser), uses)
            commit()
        }
                            ...
                            override fun onDataChange(dataSnapshot: DataSnapshot) {
                                for (ds in dataSnapshot.children) {
                                    ...
                                    if(emailtemp.toString().equals(edtemail.text.toString()))
                                    {
                                        ...
                                        with (sharedPref1.edit())
                                        {
                                            putString(getString(com.example.myworker.R.string.usernameSdt), ds.getValue<User>(User::class.java)!!.sodienthoai)
                                            commit()
                                        }
